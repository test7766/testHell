...
namespace WMSOffice.Controls.ExtraComboBox
{
    ...
    public class DropdownControl : ToolStripDropDown, IMessageFilter
    {
        ...
        public void Show(Control opener, Size preferredSize)
        {
            ...
            var args = new CalcAdaptiveDropDownSizeEventArgs();
            if (OnCalcAdaptiveDropDownSize != null)
                OnCalcAdaptiveDropDownSize(this, args);

            Width = args.CalcSize.Width > 0 ? args.CalcSize.Width : w;
            Height = args.CalcSize.Height > 0 ? args.CalcSize.Height : h;
            
            Show(m_opener, cloc, ToolStripDropDownDirection.BelowRight);
        }

        public event EventHandler<CalcAdaptiveDropDownSizeEventArgs> OnCalcAdaptiveDropDownSize;
        public class CalcAdaptiveDropDownSizeEventArgs : EventArgs
        {
            public Size CalcSize { get; set; }
        }
