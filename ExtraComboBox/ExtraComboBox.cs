...
namespace WMSOffice.Controls.ExtraComboBox
{
    /// <summary>
    ...
    /// </summary>
    public class ExtraComboBox : ComboBox
    {
        ...
        public ExtraComboBox()
        {
            ...
            m_dropDown.OnCalcAdaptiveDropDownSize += M_dropDown_OnCalcAdaptiveDropDownSize;
            onFontChanged(null, null);
        }

         [
          DefaultValue(false),
          Description("Determine when popup width is calc automatically"),
          Browsable(true),
          EditorBrowsable(EditorBrowsableState.Always),
          Category("Behavior")
       ]
        public bool UseCalcAdaptiveDropDownSize { get; set; }
        private void M_dropDown_OnCalcAdaptiveDropDownSize(object sender, DropdownControl.CalcAdaptiveDropDownSizeEventArgs e)
        {
            if (DesignMode)
            {
                return;
            ...
            if (this.UseCalcAdaptiveDropDownSize)
            {
                var size = this.GetMaxDropDownSize(this.Text);
                e.CalcSize = size;
            }
        }

        public Size GetMaxDropDownSize()
        {
            return this.GetMaxDropDownSize(null);
        }

        public Size GetMaxDropDownSize(string filter)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(filter))
                {
                    foreach (var item in this.Items)
                        filteredItems.Add(((item as DataRowView).Row)[this.DisplayMember].ToString());
                }
                else
                {
                    foreach (var item in m_suggestionList.Items)
                        if (filter.ToUpper().Contains(filter.ToUpper()))
                            filteredItems.Add((item as StringMatch).Text);
                }

                var tenderWithMaxLength = string.Empty;
                foreach (var item in filteredItems)
                    if (item.Length > tenderWithMaxLength.Length)
                        tenderWithMaxLength = item;
                
                using (var gr = this.CreateGraphics())
                {
                    var sizeF = gr.MeasureString(tenderWithMaxLength, this.Font);
                    var width = Convert.ToInt32(sizeF.Width);
