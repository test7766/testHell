...
{
    /// <summary>
    /// Окно поиска анкет входного контроля партии
    /// </summary>
    public partial class InputControlLotnSearchForm : Form
    {
        ...
        public InputControlLotnSearchForm(InputControlLotnOrdersSearchParamters pParams)
        {
            InitializeComponent();
        ...
        /// Инициализация окна
        /// </summary>
        private void InputControlLotnSearchForm_Load(object sender, EventArgs e)
        {
            InitSearchTextBoxes();
        ...
        private void CustomFilterEnability()
        {
            ...
            tbxOrderID.Enabled = chbOrderID.Checked;
        ...
        private bool ValidateData()
        {
            ...
            if (chbItem.Checked && (!Int64.TryParse(stbItem.Value, out d) || d <= 0))
                msg = "Если фильтр по товару отмечен флажком, то необходимо выбрать товар!";
            else if (chbLotNumber.Checked && String.IsNullOrEmpty(tbxLotNumber.Text))
                msg = "Если выбран фильтр по партии, то необходимо ввести код партии!";
            else if (chbVendor.Checked && (!Int64.TryParse(stbVendor.Value, out d) || d <= 0))
                msg = "Если фильтр по поставщику отмечен флажком, то необходимо выбрать поставщика!";
            else if (chbManufacturer.Checked && (!Int64.TryParse(stbManufacturer.Value, out d) || d <= 0))
                msg = "Если фильтр по производителю отмечен флажком, то необходимо выбрать производителя!";
            else if (chbOrderID.Checked && (!Int64.TryParse(tbxOrderID.Text, out d) || d <= 0))
                msg = "Если выбран фильтр по заказу, то необходимо ввести номер заказа!";
                ...
                msg = "Если выбран фильтр по ФИО провизора, то необходимо выбрать провизора в выпадающем списке!";
