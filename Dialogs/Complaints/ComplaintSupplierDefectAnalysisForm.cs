...
namespace WMSOffice.Dialogs.Complaints
{
    ...
    /// </summary>
    public partial class ComplaintSupplierDefectAnalysisForm: Form
    {
        ...
        /// <summary>
        /// Признак возможности изменить тип претензии
        /// </summary>
        private readonly bool canChangeComplaintType;
        ...
        /// </summary>
        private Data.ComplaintsExt.ComplaintInfoI2Row SelectedInfoRow
        {
            get
            {
                ...
                return (dgvComplaintDetails.SelectedRows[0].DataBoundItem as DataRowView).Row
                    as Data.ComplaintsExt.ComplaintInfoI2Row;
            }
        }
        ...
        public ComplaintSupplierDefectAnalysisForm(Data.Complaints.CurrentDocsRow pDocRow, long pSessionID, 
            bool pCanAddTakingRemainsRequest, bool pCanRequestAttachments, bool pCanChangeComplaintType)
        {
            if (pDocRow == null)
            ...
            canChangeComplaintType = pCanChangeComplaintType;
        ...
        public void checkHeaderCell_OnCheckBoxClicked(bool pState)
        {
            foreach (DataGridViewRow row in dgvComplaintDetails.Rows)
                ((row.DataBoundItem as DataRowView).Row as Data.ComplaintsExt.ComplaintInfoI2Row).IsChecked = pState;

            dgvComplaintDetails.RefreshEdit();
            complaintInfoI2BindingSource.ResetBindings(false);
        }
        ...
        private void ComplaintAnalysisForm_Load(object sender, EventArgs e)
        {
            ...
            var statusID = 0;
            if (int.TryParse(complaint.Status_ID, out statusID))
            {
                canCreateSimilarComplaint = canChangeComplaintType && statusID <= Convert.ToInt32(ComplaintsConstants.ComplaintStatusAccepting);
                cbChangeComplaintType.Enabled = canCreateSimilarComplaint;

                canPrintSupplierDefectAct = statusID >= Convert.ToInt32(ComplaintsConstants.ComplaintStatusAccepting);
        ...
        private void StartAnalysisInfoLoading()
        {
            ...
            lastWorker.RunWorkerAsync(new object[] { sessionID, complaint.Doc_ID, chbShowAll.Checked });
        }
        ...
        private void lastWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                e.Result = complaintInfoI2TableAdapter.GetData((long)param[0], (long)param[1], (bool)param[2]);
            }
            catch (Exception exc)
            {
                e.Result = exc;
        ...
        private void lastWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (sender == lastWorker)
            {
                ...
                {
                    complaintsExt.ComplaintInfoI2.Clear();
                    complaintsExt.ComplaintInfoI2.Merge(e.Result as DataTable);
                }

                dgvComplaintDetails.DataSource = complaintInfoI2BindingSource;
            ...
            try
            {
                if (SelectedInfoRow != null)
                    taTakingRemains.Fill(complaints.TakingRemains, complaint.First_Doc_ID,
                        Convert.ToInt32(SelectedInfoRow.item_id),
                ...
                try
                {
                    foreach (var row in checkedRows)
                    {
                        if (!row.IsVendor_lotNull())
                            taTakingRemains.AddRequest(sessionID, complaint.First_Doc_ID, Convert.ToInt32(row.item_id), row.Vendor_lot);
                        if (complaint.Doc_Type.Equals(ComplaintsConstants.ComplaintDocTypeRegrade,
                            ...
                            taTakingRemains.AddRequest(sessionID, complaint.First_Doc_ID, Convert.ToInt32(row.item_id), row.Vendor_Lot_Fact);
                    }
                }
                catch (Exception exc)
                {
        ...
        private List<Data.ComplaintsExt.ComplaintInfoI2Row> GetCheckedRows(out string pCheckedStr)
            {
            var result = new List<Data.ComplaintsExt.ComplaintInfoI2Row>();
            var sb = new StringBuilder();
            foreach (Data.ComplaintsExt.ComplaintInfoI2Row row in complaints.AnalysisInfo)
                if (row.IsChecked)
                {
                    if (result.Count <= 20)
                        sb.AppendLine(String.Format("({0}) {1}, серия {2}", row.item_id, row.Item_name,
                ...
                try
                {
                    foreach (var row in checkedRows)
                    {
                        using (var adapter = new Data.ComplaintsTableAdapters.AnalysisInfoTableAdapter())
                            adapter.AddAttachmentRequest(sessionID, complaint.First_Doc_ID, Convert.ToInt32(row.item_id));
                    }
        ...
        private void btnVideoControl_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty((wvmTargetPath = Environment.GetEnvironmentVariable(varEnvironment, EnvironmentVariableTarget.User))))
            ...
            try
            {
                ...
                if (this.complaintsExt.SupplierDefectActReport.Rows.Count > 0)
                {
                    ...
                    using (SaveFileDialog dlgSaveToPDF = new SaveFileDialog()
                    {
                        ...
                        {
                            report.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.RichText, dlgSaveToPDF.FileName);
                            Thread.Sleep(500);
        ...
        private static string GetPreparedFileName(string sourceFileName)
        { 
