...
namespace WMSOffice.Dialogs.Complaints
{
    ...
    /// </summary>
    public partial class ComplaintPalletAnalysisForm : Form
    {
        ...
        /// </summary>
        private Data.ComplaintsExt.AnalysisInfoPLRow SelectedInfoRow
        {
            get
            {
                ...
                return (dgvComplaintDetails.SelectedRows[0].DataBoundItem as DataRowView).Row
                    as Data.ComplaintsExt.AnalysisInfoPLRow;
            }
        }
        ...
        public ComplaintPalletAnalysisForm(Data.Complaints.CurrentDocsRow pDocRow, long pSessionID,
        ...
        public void checkHeaderCell_OnCheckBoxClicked(bool pState)
        {
            foreach (DataGridViewRow row in dgvComplaintDetails.Rows)
                ((row.DataBoundItem as DataRowView).Row as Data.ComplaintsExt.AnalysisInfoPLRow).IsChecked = pState;

            dgvComplaintDetails.RefreshEdit();
            bsAnalysisInfoPL.ResetBindings(false);
        }
        ...
        private void lastWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                using (var adapter = new Data.ComplaintsExtTableAdapters.AnalysisInfoPLTableAdapter())
        ...
        private void lastWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (sender == lastWorker)
            {
                ...
                {
                    complaintsExt.AnalysisInfoPL.Clear();
                    complaintsExt.AnalysisInfoPL.Merge(e.Result as DataTable);
                }

                dgvComplaintDetails.DataSource = bsAnalysisInfoPL;
        ...
        private List<Data.ComplaintsExt.AnalysisInfoPLRow> GetCheckedRows(out string pCheckedStr)
        {
            var result = new List<Data.ComplaintsExt.AnalysisInfoPLRow>();
            pCheckedStr = string.Empty;
            //var sb = new StringBuilder();
            //foreach (Data.ComplaintsExt.AnalysisInfoPLRow row in complaintsExt.AnalysisInfoPL)
            //    if (row.IsChecked)
            //    {
            //        if (result.Count <= 20)
        ...
        private void btnAttachmentRequest_Click(object sender, EventArgs e)
        {
            ...
            //    {
            //        foreach (var row in checkedRows)
            //            taAnalysisInfoPL.AddAttachmentRequest(sessionID, complaint.First_Doc_ID, Convert.ToInt32(row.ItemID));
